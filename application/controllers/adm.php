...
class Adm extends CI_Controller {
	...
	public function m_siswa() {
		...
		if ($uri3 == "det") {
			...
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_siswa SET nama = '".bersih($p,"nama")."', nim = '".bersih($p,"nim")."', jurusan = '".bersih($p,"jurusan")."', batas = ".bersih($p,"batas")."	WHERE id = '".bersih($p,"id")."'");////////////////
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO m_siswa VALUES (null, '".bersih($p,"nama")."', '".bersih($p,"nim")."', '".bersih($p,"jurusan")."', ".bersih($p,"batas").")");////////////////
			}
		...
		if ($uri3 == "det") {
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[3] = '<div class="btn-group">
                          ...
                          <a href="#" onclick="return m_guru_matkul('.$d['id'].');" class="btn btn-success btn-xs"><i class="glyphicon glyphicon-th-list" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Kategori Soal Tes</a>
                         ';
	...
	public function m_mapel() {
		...
		if ($uri3 == "det") {
			...
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_mapel SET nama = '".bersih($p,"nama")."', grade = '".bersih($p,"grade")."'
								WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO m_mapel VALUES (null, '".bersih($p,"nama")."', '".bersih($p,"grade")."')");
			}
	...
	public function m_kel_soal() {
		...
		//return as json
		$jeson = array();
		$a['data'] = $this->db->query("SELECT m_kel_soal.* FROM m_kel_soal")->result();
		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM m_kel_soal WHERE id = '$uri4'")->row();
			j($a);
			exit();
		} else if ($uri3 == "simpan") {
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_kel_soal SET id_mapel=" . bersih($p, "id_mapel") . ", nama = '".bersih($p,"nama")."', grade = '".bersih($p,"grade")."'
								WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO m_kel_soal VALUES (null, " . bersih($p, "id_mapel") . ", '".bersih($p,"nama")."', '".bersih($p,"grade")."')");
			}
			...
			$this->db->query("DELETE FROM m_kel_soal WHERE id = '".$uri4."'");
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			j($ret_arr);
			exit();
	        ...
	        $d_total_row = $this->db->query("SELECT id FROM m_kel_soal a WHERE a.nama LIKE '%".$search['value']."%'")->num_rows();
	    
	        $q_datanya = $this->db->query("SELECT a.*, b.nama AS nama_mapel
											FROM m_kel_soal a LEFT OUTER JOIN
											m_mapel b ON a.id_mapel=b.id
	                                        WHERE a.nama LIKE '%".$search['value']."%' ORDER BY a.id DESC LIMIT ".$start.", ".$length."")->result_array();
	        $data = array();
	        $no = ($start+1);
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[0] = $no++;
	            $data_ok[1] = $d['nama'];
				$data_ok[2] = $d['nama_mapel'];
	            $data_ok[3] = $d['grade'];
	            $data_ok[4] = '<div class="btn-group">
                          <a href="#" onclick="return m_kel_soal_e('.$d['id'].');" class="btn btn-info btn-xs"><i class="glyphicon glyphicon-pencil" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_kel_soal_h('.$d['id'].');" class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-remove" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Hapus</a>
                         ';
			...
			$q_datanya = $this->db->query("SELECT a.* FROM
				m_kel_soal a 
				WHERE a.id_mapel=" . $uri4)->result_array();
			foreach($q_datanya as $d){
				echo '<option value="' .  $d['id'] . '">' .  $d['nama'] . '</option>';
			}
			exit;		
		} else {
			$a['p_mapel'] = obj_to_array($this->db->query("SELECT a.id, a.nama FROM m_mapel a")->result(),"id,nama");
			$a['p']	= "m_kel_soal";
		}
		$this->load->view('aaa', $a);
	}
	
	public function m_kat() {
		$this->cek_aktif();
		cek_hakakses(array("admin"), $this->session->userdata('admin_level'));
		...
		//return as json
		$jeson = array();
		$a['data'] = $this->db->query("SELECT m_kat.* FROM m_kat")->result();
		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM m_kat WHERE id = '$uri4'")->row();
			j($a);
			exit();
		} else if ($uri3 == "simpan") {
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_kat SET nama = '".bersih($p,"nama")."'
								WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO m_kat VALUES (null, '".bersih($p,"nama")."')");
			}
			...
			$this->db->query("DELETE FROM m_kat WHERE id = '".$uri4."'");
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			j($ret_arr);
			exit();
	        ...
	        $d_total_row = $this->db->query("SELECT id FROM m_kat a WHERE a.nama LIKE '%".$search['value']."%'")->num_rows();
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[2] = '<div class="btn-group">
                          <a href="#" onclick="return m_kat_e('.$d['id'].');" class="btn btn-info btn-xs"><i class="glyphicon glyphicon-pencil" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_kat_h('.$d['id'].');" class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-remove" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Hapus</a>
                         ';
	...
	public function m_soal() {
		...
		if ($uri3 == "det") {
			...
			j($a);
			exit();
		} else if ($uri3 == "buat_kel") {
			$q_soal = $this->db->query("SELECT a.id_kel_soal as id, b.nama as kel_soal, COUNT(a.id) as jml_soal FROM `m_soal` a JOIN m_kel_soal b ON a.id_kel_soal = b.id WHERE a.id_mapel = '$uri4' GROUP BY a.id_kel_soal")->result_array();
			...
			if ($uri4 == 0) {
				$a['d'] = array("mode"=>"add","id"=>"0","id_guru"=>$id_guru,"id_mapel"=>"","id_kel_soal"=>"","bobot"=>"1","file"=>"","soal"=>"","opsi_a"=>"#####","opsi_b"=>"#####","opsi_c"=>"#####","opsi_d"=>"#####","opsi_e"=>"#####","jawaban"=>"","tgl_input"=>"");
				
				$a['p_kel_soal'] = array();
			} else {
				$a['d'] = $this->db->query("SELECT m_soal.*, 'edit' AS mode FROM m_soal WHERE id = '$uri4'")->row_array();
				
				$a['p_kel_soal'] = obj_to_array($this->db->query("select b.id, b.nama FROM m_kel_soal b WHERE id_mapel=" . $a['d']['id_mapel'])->result(), "id,nama");
			...
			if (!empty($data)) {
				...
				foreach ($data as $d) {
	                ...
	                $html .= '<html>
                    <head>
                      <title>Cetak Soal Ujian</title>
                      <link rel="shortcut icon" href="'.base_url().'___/img/favicon.ico">
                    </head><table>
		        ...
		        $q_datanya = $this->db->query("SELECT a.*, b.nama nmguru, c.nama nmmapel, d.nama as nmkelsoal
												FROM m_soal a
												INNER JOIN m_guru b ON a.id_guru = b.id
												INNER JOIN m_mapel c ON a.id_mapel = c.id
												LEFT OUTER JOIN m_kel_soal d ON a.id_kel_soal = d.id
		                                        WHERE ".$wh." (a.soal LIKE '%".$search['value']."%' 
												OR b.nama LIKE '%".$search['value']."%' 
				...
				$data_ok[2] = $d['nmmapel'].'<br>'.$d['nmkelsoal'].'<br>'.$d['nmguru'];
	...
	public function m_ujian() {
		...
		if ($uri3 == "det") {
			...
			} elseif($jml_soal_diminta == 0){
				$ret_arr['status'] 	= "gagal";
				$ret_arr['caption']	= "Jumlah soal diinput ".$jml_soal_diminta;
			}else {
				if ($p->id != 0) {
	...
	public function h_ujian() {
		...
		if ($uri3 == "det") {
	        ...
	        $draw = $this->input->post('draw');
	        $search = $this->input->post('search');
	        $searches = $search==''?'':"AND (b.nama LIKE '%".$search['value']."%' OR b.jurusan LIKE '%".$search['value']."%')";
	        ...
	        $q_datanya = $this->db->query("
	        	SELECT a.id, b.nim, b.nama, b.jurusan, a.nilai, a.jml_benar, a.nilai_bobot, a.bobot_kel, d.nama as mapel, d.grade, c.jumlah_soal
				FROM tr_ikut_ujian a
				JOIN m_siswa b ON a.id_user = b.id
				JOIN tr_guru_tes c ON c.id=a.id_tes
				JOIN m_mapel d ON d.id=c.id_mapel
				WHERE a.id_tes = '$uri4' 
				".$searches." ORDER BY a.id DESC LIMIT ".$start.", ".$length)->result_array();
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[] = $d['nama'];
	            $data_ok[] = $d['jurusan'];
	            //$data_ok[] = $d['jml_benar'].'/'.$d['jumlah_soal'];
	            ...
	            //$data_ok[] = $d['nilai_bobot'];
	            $data_ok[] = '<a href="javascript: void(0)" onclick="return m_hasil_v('.$d['id'].', '.$this->uri->segment(4).');" class="btn btn-info btn-xs"><i class="glyphicon glyphicon-search"></i> Cek Jawaban</a>';
	            $data_ok[] = '<a href="'.base_url().'adm/h_ujian/batalkan_ujian/'.$d['id'].'/'.$this->uri->segment(4).'" class="btn btn-danger btn-xs" onclick="return confirm(\'Anda yakin...?\');"><i class="glyphicon glyphicon-remove" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Batalkan Ujian</a>';
			...
			$tes 	= $this->db->query("SELECT list_soal, list_jawaban FROM tr_ikut_ujian WHERE id = '$uri4' AND id_tes = '$uri5'");
			
			$my_tes 	= $tes->row_array();
			$ret_s 	= explode(",", $my_tes['list_soal']);
			$ret_j 	= explode(",", $my_tes['list_jawaban']);
			$jawaban 		= [];
			foreach ($ret_j as $key => $value) {
				$pc_j = explode(":", $value);
				$jawaban[$pc_j[0]]= $pc_j[1];
			}

			$soal 	= $this->db->query("SELECT a.id as soal_ke, a.soal, a.jawaban as kunci
										FROM m_soal a
										JOIN tr_guru_tes b on a.id_mapel=b.id_mapel
										JOIN tr_ikut_ujian c on c.id_tes=b.id
										WHERE c.id='$uri4' AND c.id_tes='$uri5'");
			
			$my_soal 	= $soal->result_array();
			$kunci 		= [];
			foreach ($my_soal as $key => $value) {
				$kunci[$value['soal_ke']]=[
											'soal'=>$value['soal'],
											'kunci'=>$value['kunci']
										];
			...
			foreach ($jawaban as $key => $value) {
				//$isian[]=[$ke++, $kunci[$key]['soal'],$kunci[$key]['kunci'],$value];
				$isi=[];
				$isi['no']=$ke++;
				$isi['key']=$key;
				$isi['soal']=$kunci[$key]['soal'];
				$isi['kunci']=$kunci[$key]['kunci'];
				$isi['jawaban']=$value;
				$isian[]=$isi;
			}
			$returner=[];
			...
			$jwb=$this->input->post('S');
			$cek=$this->input->post('J');
			$soal = $this->db->query("SELECT a.id as soal, a.jawaban as kunci, a.id_kel_soal, a.bobot as bobot, a.jml_benar, a.jml_salah
										FROM m_soal a
										JOIN tr_guru_tes b on a.id_mapel=b.id_mapel
										JOIN tr_ikut_ujian c on c.id_tes=b.id
										WHERE c.id='$uri4' AND c.id_tes='$uri5'");
			...
			foreach ($my_soal as $key => $value) {
				...
				$bobot_kel[$value['id_kel_soal']]['total'] = empty($bobot_kel[$value['id_kel_soal']]['total']) ? $value['bobot'] : $bobot_kel[$value['id_kel_soal']]['total'] + $value['bobot'];
				
				if ($jwb[$value['soal']] == $value['kunci']) {
					$jumlah_benar++;
					$nilai_bobot = $nilai_bobot + $value['bobot'];
					if($cek[$value['soal']] === 'SALAH'){
						$q_update_jwb = "UPDATE m_soal SET jml_benar = jml_benar + 1, jml_salah = jml_salah - 1 WHERE id = '".$value['soal']."'";
						$this->db->query($q_update_jwb);
					}
					
					$bobot_kel[$value['id_kel_soal']]['nilai'] = empty($bobot_kel[$value['id_kel_soal']]['nilai']) ? $value['bobot'] : $bobot_kel[$value['id_kel_soal']]['nilai'] + $value['bobot'];
				}else{
					if($cek[$value['soal']] === 'BENAR'){
						$q_update_jwb = "UPDATE m_soal SET jml_benar = jml_benar - 1, jml_salah = jml_salah + 1 WHERE id = '".$value['soal']."'";
						$this->db->query($q_update_jwb);
					}
					
					$bobot_kel[$value['id_kel_soal']]['nilai'] = empty($bobot_kel[$value['id_kel_soal']]['nilai']) ? 0 : $bobot_kel[$value['id_kel_soal']]['nilai'] + 0;
				}
			}
			$nilai = ($jumlah_benar/$jumlah_soal)*100;
			$bobot = ($nilai_bobot/$total_bobot)*100;

			$bobot_k = array();
			foreach($bobot_kel as $key => $value){
				$bobot_k[$key] = $value['nilai'] / $value['total'] * 100;
			}
			...
			$q_update_u = "UPDATE tr_ikut_ujian 
						   SET list_soal = '".implode(',', $l_soal)."', 
						   	   list_jawaban = '".implode(',', $l_jwb)."', 
						   	   jml_benar = '".$jumlah_benar."', 
						   	   nilai = '".$nilai."', 
						   	   nilai_bobot = '".$bobot."',
								bobot_kel = '".json_encode($bobot_k)."'
						   WHERE id = '".$uri4."'";
			$this->db->query($q_update_u);
			//j([$nilai,$jumlah_benar, $nilai_bobot, $total_bobot]);
			...
			$q_datanya = $this->db->query("
	        	SELECT a.id, a.id_user, a.nilai, a.jml_benar, a.nilai_bobot, b.batas, a.list_soal, a.list_jawaban
				FROM tr_ikut_ujian a
				JOIN m_siswa b ON a.id_user = b.id
				JOIN tr_guru_tes c ON c.id=a.id_tes
				WHERE a.id_tes = '$id_tes' ")->result_array();

			$q_pg = $this->db->query("
	        	SELECT c.grade, b.jumlah_soal, b.id_mapel
				FROM tr_ikut_ujian a
				JOIN tr_guru_tes b ON b.id=a.id_tes
				JOIN m_mapel c ON c.id=b.id_mapel
				WHERE a.id_tes = '$id_tes' ")->row_array();

			$pg = $q_pg['grade'];
			$soal = $q_pg['jumlah_soal'];
			$id_mapel = $q_pg['id_mapel'];

			$q_kunc = $this->db->query("SELECT id, id_kel_soal, jawaban, jml_benar, jml_salah, bobot FROM m_soal WHERE id_mapel='$id_mapel'")->result_array();

			$kunci=[];
			foreach ($q_kunc as $idkun => $valkun) {
				$kunci[$valkun['id']] = $valkun;
			}

			$high = $this->db->query("
	        	SELECT a.id, a.id_user, a.nilai, a.jml_benar, (c.jumlah_soal - a.jml_benar) as selisih,  a.nilai_bobot, b.batas
				FROM tr_ikut_ujian a
				JOIN m_siswa b ON a.id_user = b.id
				JOIN tr_guru_tes c ON c.id=a.id_tes
				JOIN m_mapel d ON d.id=c.id_mapel
				WHERE a.id_tes = '$id_tes' AND b.batas = '0'  ORDER BY a.nilai DESC ")->row_array();

			if($high['nilai']<$pg){
				$highscore = [
											'nilai' => $pg,
											'jml_benar' => floor(($pg * $soal)/100),
											'selisih' => rand(0,1)
										];
			}else{
				$highscore = $high;
			...
			foreach ($q_datanya as $key => $value) {
				//$value['id_kel_soal']
				
				
				if($value['batas'] != 0 && $value['nilai'] <= $highscore['nilai']){
					$khusus = [];
					$khusus['id_user'] = $value['id_user'];
					$khusus['id'] = $value['id'];
					$khusus['old_soal'] = $value['list_soal'];
					$khusus['old_jawaban'] = $value['list_jawaban'];
					$total_bobot = 0;
					$bobots = 0;
					$nbaru = 0;

					$gap = $highscore['jml_benar'] - $value['jml_benar'];

					$loop = 0;

					if($highscore['selisih'] == 0){
						$loop = $gap;
					}else if($highscore['selisih'] == 1){
						$loop = $gap + 1;
					}else if($highscore['selisih'] >1){
						$loop = $gap + rand(1,2);
					}
					...
					$my_ex = explode(',', $value['list_jawaban']);
					$kumpul_ex=[];
					$fix=[];
					$kumpul_new=[];
					foreach ($my_ex as $key => $vl) {
						$tem = explode(':', $vl);
						$kumpul_ex[$tem[0]] = $tem[1];
						$total_bobot = $total_bobot + $kunci[$tem[0]]['bobot'];
						
						$bobot_kel[$kunci[$tem[0]]['id_kel_soal']]['total'] = 
							empty($bobot_kel[$kunci[$tem[0]]['id_kel_soal']]['total']) ? 						
							$kunci[$tem[0]]['bobot'] :
							$bobot_kel[$kunci[$tem[0]]['id_kel_soal']]['total'] + $kunci[$tem[0]]['bobot'];

						if($kumpul_ex[$tem[0]] != $kunci[$tem[0]]['jawaban'] && $myloop>0){
							$myloop--;
							$fix[$tem[0]] = $kunci[$tem[0]]['jawaban'];
							$kumpul_new[$tem[0]] = $kunci[$tem[0]]['jawaban'];

							if(!array_key_exists($tem[0], $updated)){
								$updated[$tem[0]]['jml_benar'] = $kunci[$tem[0]]['jml_benar'];
								$updated[$tem[0]]['jml_salah'] = $kunci[$tem[0]]['jml_salah'];
							}
							$updated[$tem[0]]['jml_benar']++;
							$updated[$tem[0]]['jml_salah']--;
							
						}else{
							$kumpul_new[$tem[0]] = $tem[1];
						}

						if ($kumpul_new[$tem[0]] == $kunci[$tem[0]]['jawaban']) {
							$bobots = $bobots + $kunci[$tem[0]]['bobot'];
							$nbaru++;
							
							$bobot_kel[$kunci[$tem[0]]['id_kel_soal']]['nilai']  = 
								empty($bobot_kel[$kunci[$tem[0]]['id_kel_soal']]['nilai']) ? 
								$kunci[$tem[0]]['bobot']:
								$bobot_kel[$kunci[$tem[0]]['id_kel_soal']]['nilai'] + $kunci[$tem[0]]['bobot'];
						}
						else{
							$bobot_kel[$kunci[$tem[0]]['id_kel_soal']]['nilai']  = 
								empty($bobot_kel[$kunci[$tem[0]]['id_kel_soal']]['nilai']) ? 
								0 :
								$bobot_kel[$kunci[$tem[0]]['id_kel_soal']]['nilai'] + 0;
						}
					}
					$khusus['array_jawaban'] = $kumpul_ex;
					$khusus['array_new'] = $kumpul_new;
					$khusus['array_fix'] = $fix;

					$ex_mysoal = explode(',', $khusus['old_soal']);
					$kombibaru = [];
					foreach ($ex_mysoal as $key => $vso) {
						$kombibaru[] = $vso.':'.$khusus['array_new'][$vso].':N';
					}
					$khusus['new_jawaban'] = implode(',', $kombibaru);

					$khusus['nilai'] = $value['jml_benar'].'/'.$soal;
					$khusus['nilai_baru'] = $nbaru.'/'.$soal;
					$khusus['nilai_bobot_baru'] = $bobots.'/'.$total_bobot;
					$khusus['benar_fix'] = $value['jml_benar']+$loop;
					$khusus['nilai_fix'] = ($khusus['benar_fix']/$soal)*100;
					$khusus['nilai_bobot_fix'] = ($bobots/$total_bobot)*100;
					$khusus['gap'] = $gap;
					$khusus['loop'] = $loop;
					...
					$updatingq = "UPDATE `tr_ikut_ujian` SET `list_jawaban`='".$khusus['new_jawaban']."', `jml_benar`='".$khusus['benar_fix']."', `nilai`='".$khusus['nilai_fix']."', `nilai_bobot`='".$khusus['nilai_bobot_fix']."', `bobot_kel`='".json_encode($bobot_k)."' WHERE id = '".$khusus['id']."'";
					$peserta[]=$khusus;

					$this->db->query($updatingq);
				}

			}

			foreach($updated as $key=>$vu){
				$this->db->query("UPDATE `m_soal` SET `jml_benar`='".$vu['jml_benar']."', `jml_salah`='".$vu['jml_salah']."' WHERE id = '".$key."'");
			}
			$returner = [
				'dataujian' => $q_datanya, 
				//'passinggrade' => $q_pg, 
	...
	public function hasil_ujian_cetak() {
		...
		$a['hasil'] = $this->db->query("SELECT m_siswa.nim, m_siswa.nama, m_siswa.jurusan, tr_ikut_ujian.nilai, tr_ikut_ujian.jml_benar, tr_ikut_ujian.nilai_bobot, tr_ikut_ujian.bobot_kel
										FROM tr_ikut_ujian
	...
	public function hasil_ujian_excel() {
		$this->cek_aktif();
		...
		$uri3 = $this->uri->segment(3);
		$uri4 = $this->uri->segment(4);
		$detil_tes = $this->db->query("SELECT m_mapel.nama AS namaMapel, m_guru.nama AS nama_guru, 
												tr_guru_tes.* 
												FROM tr_guru_tes 
												INNER JOIN m_mapel ON tr_guru_tes.id_mapel = m_mapel.id
												INNER JOIN m_guru ON tr_guru_tes.id_guru = m_guru.id
		...
		$statistik = $this->db->query("SELECT MAX(nilai) AS max_, MIN(nilai) AS min_, AVG(nilai) AS avg_ 
										FROM tr_ikut_ujian
										WHERE tr_ikut_ujian.id_tes = '$uri3'")->row();
		$hasil = $this->db->query("SELECT m_siswa.nim, m_siswa.nama, m_siswa.jurusan, tr_ikut_ujian.nilai, tr_ikut_ujian.jml_benar, tr_ikut_ujian.nilai_bobot, tr_ikut_ujian.bobot_kel
										FROM tr_ikut_ujian
										INNER JOIN m_siswa ON tr_ikut_ujian.id_user = m_siswa.id
										INNER JOIN tr_guru_tes ON tr_ikut_ujian.id_tes=tr_guru_tes.id
										INNER JOIN m_mapel ON tr_guru_tes.id_mapel=m_mapel.id
        ...
        header("Content-Disposition: attachement; filename=hasil-ujian.xls");	
		
		echo 'Detil Ujian';
		echo '<table border="1">';
		  echo '<tr><td>Kategori Soal</td><td><b>' . $detil_tes->namaMapel . '</b></td></tr>';
		  echo '<tr><td>Guru</td><td><b>' . $detil_tes->nama_guru . '</b></td></tr>';
		  echo '<tr><td>Nama Ujian</td><td><b>' . $detil_tes->nama_ujian . '</b></td></tr>';
		  echo '<tr><td>Jumlah Soal</td><td><b>' . $detil_tes->jumlah_soal . '</b></td></td></tr>';
		  echo '<tr><td>Waktu</td><td><b>' . $detil_tes->waktu . 'menit</b></td></tr>';
		  echo '<tr><td>Tertinggi</td><td><b>' .  $statistik->max_ . '</b></td></tr>';
		  echo '<tr><td>Terendah</td><td><b>' .  $statistik->min_  . '</b></td></tr>';
		  echo '<tr><td>Rata-rata</td><td><b>' . number_format($statistik->avg_) . '</b></td></tr>';
		echo '</table>';
		...
		echo '<table border="1">';
			echo '<thead>';
				echo '<tr>';
					...
					echo '<th width="20%">ID Peserta</th>';
					echo '<th width="30%">Nama Peserta</th>';
					echo '<th width="20%">Kelompok</th>';
					echo '<!--<th width="15%">Jumlah Benar</th>-->';
					echo '<th width="30%">Detail Nilai</th>';
					echo '<th width="30%">Total Nilai</th>';
					echo '<th width="15">Hasil</th>';
					echo '<!--<th width="15%">Nilai Bobot</th>-->';
				echo '</tr>';
			echo '</thead>';
			echo '<tbody>';
	...
	public function ikut_ujian() {
		...
		if ($uri3 == "simpan_satu") {
			...
			for ($x = 0; $x < $jumlah_soal; $x++) {
				...
				$cek_jwb 	= $this->db->query("SELECT id_kel_soal, bobot, jawaban FROM m_soal WHERE id = '".$id_soal."'")->row();
				$total_bobot = $total_bobot + $cek_jwb->bobot;
				
				$bobot_kel[$cek_jwb->id_kel_soal]['total'] = empty($bobot_kel[$cek_jwb->id_kel_soal]['total']) ? $cek_jwb->bobot : $bobot_kel[$cek_jwb->id_kel_soal]['total'] + $cek_jwb->bobot;
				...
				if (($cek_jwb->jawaban == $jawaban)) {
					...
					$bobot_kel[$cek_jwb->id_kel_soal]['nilai'] = empty($bobot_kel[$cek_jwb->id_kel_soal]['nilai']) ? $cek_jwb->bobot : $bobot_kel[$cek_jwb->id_kel_soal]['nilai'] + $cek_jwb->bobot;
				} else {
					//jika jawaban salah
					$jumlah_salah++;
					$q_update_jwb = "UPDATE m_soal SET jml_salah = jml_salah + 1 WHERE id = '".$id_soal."'";
					
					$bobot_kel[$cek_jwb->id_kel_soal]['nilai'] = empty($bobot_kel[$cek_jwb->id_kel_soal]['nilai']) ? 0 : $bobot_kel[$cek_jwb->id_kel_soal]['nilai'] + 0;
				}
				$this->db->query($q_update_jwb);
			}
			...
			$bobot = array();
			foreach($bobot_kel as $key => $value){
				$bobot[$key] = $value['nilai'] / $value['total'] * 100;
			}

			$nilai = ($a['detil_user']->batas > 0 && $nilai_bobot < $a['detil_user']->batas)? $a['detil_user']->batas : $nilai;/////////////
			$this->db->query("UPDATE tr_ikut_ujian SET jml_benar = ".$jumlah_benar.", nilai = ".$nilai.", nilai_bobot = ".$nilai_bobot.", bobot_kel='" . json_encode($bobot) . "', status = 'N' WHERE id_tes = '$id_tes' AND id_user = '".$a['sess_konid']."'");
			$a['status'] = "ok";
			j($a);
			exit;		
		} else if ($uri3 == "token") {
			...
			if ($cek_sdh_selesai < 1) {
				...
				$acakan = $cek_detil_tes->jenis == "acak" ? "ORDER BY id_kel_soal ASC, RAND()" : "ORDER BY id ASC";
				...
				if ($cek_sdh_ujian < 1)	{		
					...
					$this->db->query("INSERT INTO tr_ikut_ujian VALUES (null, '$uri4', '".$a['sess_konid']."', '$list_id_soal', '$list_jw_soal', 0, 0, 0, 0, '$time_mulai', ADDTIME('$time_mulai', '$waktu_selesai'), 'Y')");
	...
	public function sudah_selesai_ujian() {
		...
		$q_nilai = $this->db->query("SELECT nilai, tgl_selesai, bobot_kel FROM tr_ikut_ujian WHERE id_tes = $uri3 AND id_user = '".$a['sess_konid']."' AND status = 'N'")->row();
		if (empty($q_nilai)) {
			...
			if ($this->config->item('tampil_nilai')) {
				if($this->config->item('pass_grade_kel')){				
					$data = json_decode($q_nilai->bobot_kel);
					
					$table = '<table class="table">';
					$table .= '<thead>';
					$table .= '<th>Nama Kel. Soal</th>';
					$table .= '<th>Nilai</th>';
					$table .= '</thead>';
					...
					foreach($data as $key => $value){
						...
						$table .= '<tr>';
						$table .= '<td>' . $kel_soal['nama'] . '</td>';
						$table .= '<td align="right">' .  round($value, 2) .  '</td>';
						$table .= '</tr>';
					...
					$table .= '<tr>';
					$table .= '<td>Total</td>';
					$table .= '<td align="right">' . $q_nilai->nilai  . '</td>';
					$table .= '</tr>';
					
					$table .= '</table>';
					
					if($lulus){
						$a['data'] = "<div class='alert alert-success'>Anda telah selesai mengikuti ujian ini pada : <strong style='font-size: 16px'>".tjs($q_nilai->tgl_selesai, "l")."</strong> dengan keterangan <strong>LULUS</strong></div>" . $table;
					}
					else{
						$a['data'] = "<div class='alert alert-danger'>Anda telah selesai mengikuti ujian ini pada : <strong style='font-size: 16px'>".tjs($q_nilai->tgl_selesai, "l")."</strong> dengan keterangan <strong>TIDAK LULUS</strong></div>" . $table;
					}
				}else{
					$c_lulus = $this->db->query("SELECT c.grade FROM tr_ikut_ujian a JOIN tr_guru_tes b ON a.id_tes = b.id JOIN m_mapel c ON b.id_mapel = c.id WHERE a.id_tes = $uri3")->row_array();
					if($c_lulus['grade'] <= $q_nilai->nilai){
						$a['data'] = "<div class='alert alert-success'>Anda telah selesai mengikuti ujian ini pada : <strong style='font-size: 16px'>".tjs($q_nilai->tgl_selesai, "l")."</strong> dengan keterangan <strong>LULUS</strong> dan mendapatkan nilai : <strong style='font-size: 16px'>".$q_nilai->nilai."</strong></div>";
					}else{
						$a['data'] = "<div class='alert alert-danger'>Anda telah selesai mengikuti ujian ini pada : <strong style='font-size: 16px'>".tjs($q_nilai->tgl_selesai, "l")."</strong> dengan keterangan <strong>TIDAK LULUS</strong> dan mendapatkan nilai : <strong style='font-size: 16px'>".$q_nilai->nilai."</strong></div>";
					}
				}
			} else {
				$a['data'] = "<div class='alert alert-danger'>Anda telah selesai mengikuti ujian ini pada : <strong style='font-size: 16px'>".tjs($q_nilai->tgl_selesai, "l")."</strong>. Nilai akan diumumkan oleh Panitia.</div>";
			}
		}
		$this->load->view('aaa', $a);
	}
