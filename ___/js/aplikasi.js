$(document).ready(function() {
	...
	} else if (url == "m_kel_soal") {
		pagination("datatabel", base_url+"adm/m_kel_soal/data", []);			
	} else if (url == "m_mapel") {
...
function m_ujian_e(id) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			__my_kel_soal($('#mapel').val())
			__ambil_kel_soal(data.id)
...
function slider_value(el) {
	var next = el.nextElementSibling
	next.value = el.value

	var jml = $('.persoal').map((i, e) => e.value).get();
	var total = 0;
	$.each(jml, function (i,v) {
		total = parseInt(total) + parseInt(v)
	})
	$("#jumlah_soal").val(total);
}

function __my_kel_soal(id) {
	var idmapel = (id == '')? 0 : id

	$.ajax({
		type: "GET",
		url: base_url+"adm/m_soal/buat_kel/"+idmapel,
		success: function(data) {
			var isi=''
			$('#soalan').empty()
			
			$.each(data, function (i,v) {
				//console.log(v.kel_soal)
				isi += '<div style="margin-bottom:10px"><div>'+v.kel_soal+'</div><div><div style="display: flex;gap: 10px;"><input type="range" min="0" max="'+v.jml_soal+'" value="0" step="1" oninput="return slider_value(this)"><input type="number" class="persoal" value="0" readonly name="'+v.id+'" style="width:50px"></div></div></div>'
...
function __ambil_kel_soal(id) {
	$.ajax({
		type: "GET",
		url: base_url+"adm/m_soal/ambil_kel/"+id,
		success: function(data) {
	...
	$.ajax({
		...
		success: function(data) {
			if (data.status == "ok") {
				...
				hate += '<div class="modal-footer"><button class="btn btn-primary" type="submit"><i class="fa fa-check"></i> Simpan</button><button class="btn" data-dismiss="modal" aria-hidden="true"><i class="fa fa-minus-circle"></i> Tutup</button></div></form></div></div></div>';
...
//kel_soal
function m_kel_soal_mapel(id) {	
	$('#id_kel_soal').html('');
		
	$.ajax({
		type: "GET",
		url: base_url+"adm/m_kel_soal/mapel/"+id,
		success: function(data) {
			$('#id_kel_soal').html(data);
		}
	});
	return false;
}

function m_kel_soal_e(id) {
	$("#m_kel_soal").modal('show');
	$.ajax({
		type: "GET",
		url: base_url+"adm/m_kel_soal/det/"+id,
		success: function(data) {
			$("#id").val(data.id);
			$("#nama").val(data.nama);
...
function m_kel_soal_s() {
	var f_asal	= $("#f_kel_soal");
	var form	= getFormData(f_asal);
	...
	$.ajax({		
		type: "POST",
		url: base_url+"adm/m_kel_soal/simpan",
		data: JSON.stringify(form),
		dataType: 'json',
		contentType: 'application/json; charset=utf-8'
	}).done(function(response) {
		if (response.status == "ok") {
			window.location.assign(base_url+"adm/m_kel_soal"); 
		} else {
			console.log('gagal');
		}
	});
...
function m_kel_soal_h(id) {
	if (confirm('Anda yakin..?')) {
		$.ajax({
			type: "GET",
			url: base_url+"adm/m_kel_soal/hapus/"+id,
			success: function(response) {
				if (response.status == "ok") {
					window.location.assign(base_url+"adm/m_kel_soal"); 
				} else {
...
var t_review= $('#tabelreview').DataTable({
	paging:false,
	searching: false,
	info: false,
	ordering: false,
...
var t_revu= $('#tabelrevu').DataTable({
	paging:false,
	searching: false,
	info: false,
	ordering: false,
	columnDefs:[
		...
		},
		{
			...
			render: function (data, type, row, meta) {
						if(type === 'display'){
							var input = '<input name="S['+row.key+']" value="'+data+'" >'
							if(data == row.kunci){
								return input + '<input name="J['+row.key+']" value="BENAR" hidden >';
							}else{
								return input + '<input name="J['+row.key+']" value="SALAH" hidden >';
							}
						}
						return data;
					}
...
function m_hasil_v(id, ujian) {
	$("#u_hasil").modal('hide');
	$("#m_hasil").modal('show');
	$.ajax({
		type: "GET",
		url: base_url+"adm/h_ujian/review/"+id+"/"+ujian,
		success: function(data) {
			$.each(data.data, function(i,v){
				t_review.row.add(v)
			})
			t_review.draw();
			$('.nextbtn').attr('onclick','return m_hasil_u('+data.id+', '+data.ujian+')');
		}
	});
	return false;
}

$('#m_hasil').on('hidden.bs.modal', function(){
	t_review.clear().draw()
	var url3 = get_url((parseInt(uri_js)+2));
	pagination("datatabel", base_url+"adm/h_ujian/data_det/"+url3, []);
})

function m_hasil_u(id,ujian) {
	$("#m_hasil").modal('hide');
	$('#u_hasil').modal('show')
	$('.backbtn').attr('onclick','return m_hasil_v('+id+', '+ujian+')');
	$('.updatebtn').attr('onclick','return hasil_u('+id+', '+ujian+')');
	$('input#id').val(id)
	$('input#ujian').val(ujian)
	$('#printreview').attr('href', base_url+'adm/hasil_ujian_cetak/'+id)
	$.ajax({
		type: "GET",
		url: base_url+"adm/h_ujian/review/"+id+"/"+ujian,
		success: function(data) {
...
$('#u_hasil').on('hidden.bs.modal', function(){
	t_revu.clear().draw()
})

function hasil_u(id,ujian) {
	var data = $('input').serialize();
	$.ajax({
		type: "post",
		url: base_url+"adm/h_ujian/revu/"+id+"/"+ujian,
		data: data,
		success: function(data) {
			m_hasil_v(id,ujian)
		}
...
function preset(id) {
	$.ajax({
		type: "post",
		url: base_url+"adm/h_ujian/preset",
		data: {id:id},
